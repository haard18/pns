...
export class Config {
  ...
  // Indexer configuration - optimized for fast scanning with Alchemy RPC
  static readonly indexer = {
    scanIntervalMs: parseInt(process.env.INDEXER_SCAN_INTERVAL_MS || '30000', 10), // 30 seconds
    batchSize: parseInt(process.env.INDEXER_BATCH_SIZE || '2000', 10), // Large batches for speed
    logChunkSize: parseInt(process.env.INDEXER_LOG_CHUNK_SIZE || '2000', 10), // Large chunks for eth_getLogs
