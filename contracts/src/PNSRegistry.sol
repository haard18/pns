...
contract PNSRegistry is AccessControl, Initializable, UUPSUpgradeable, ReentrancyGuard {
    ...
    modifier nameNotExpired(bytes32 nameHash) {
        require(
            records[nameHash].expiration > block.timestamp,
            "PNS: Name has expired"
        );
        _;
    }
     ...
     */
    function getSubdomainOwner(bytes32 parent, bytes32 label)
        external
        view
        returns (address)
    {
